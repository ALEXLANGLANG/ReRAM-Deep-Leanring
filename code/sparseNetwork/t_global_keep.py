...
#    utils.checkdir(f"/local/data/Xian/saves/{args.arch_type}/{args.dataset}_norm/")
#    torch.save(model, f"/local/data/Xian/saves/{args.arch_type}/{args.dataset}/initial_state_dict_{args.prune_type}.pt")
    ...
    list_all =[]
    list_bestacc =[]
    model = torch.load(f"/local/data/Xian/saves/{args.arch_type}/{args.dataset}/24_model_{args.prune_type}.pth.tar")
    for _ite in range(25, ITERATION):
        if not _ite == 0:
...
for arch_type in ["alexnet", "vgg16","vgg19", "resnet18"]:
    args = argument(end_iter =80,arch_type =arch_type,dataset = 'cifar10',prune_percent  = 60,lr= 0.05,prune_iterations = 30)
