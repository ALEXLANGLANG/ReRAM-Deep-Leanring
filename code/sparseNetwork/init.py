...
def train_main(args):
    ...
    if args.dataset == "mnist":
        ...
        traindataset = datasets.ImageNet('/local/data/Xian', train=True, download=True,transform=transform_train)
        testdataset = datasets.ImageNet('/local/data/Xian', train=False, transform=transform_test)
        from archs.cifar10 import vgg,resnet #,AlexNet
    ...
    for _ite in range(args.start_iter, ITERATION):
        if not _ite == 0:
            ...
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.95)
#
#            if args.arch_type == "vgg19":
#                optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum = 0.9, weight_decay=0.0005)
#                criterion = torch.nn.CrossEntropyLoss().to(device)
#                scheduler = torch.optim.lr_scheduler.StepLR(optimizer,milestones=[80,120], gamma=0.1)
        print(f"\n--- Pruning Level [{_ite}/{ITERATION}]: --- Iterative: unpruned: {0.8**_ite*100.}")

        # Print the table of Nonzeros in each layer
        #res = utils.print_nonzeros(model,list_all)
        comp1,list_all = 1,[] #res[0], res[1]
        comp[_ite] = comp1
        ...
        utils.checkdir(f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/")
        torch.save(model_tmp,f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/{_ite}_model_{args.prune_type}.pth.tar")
...
#    arr = np.append(arr, np.array([list_compression]), axis=0)
#    df = pd.DataFrame(arr)
#    df.to_csv(f'{os.getcwd()}/stats/{args.arch_type}{args.dataset}.csv')
#
...
class argument:
    def __init__(self, lr=1.2e-3,batch_size = 128,start_iter = 0,end_iter = 100,print_freq = 1,
        ...
        self.arch_type = arch_type # "fc1 | lenet5 | alexnet | vgg16 | vgg11 | vgg19 |resnet18 | densenet121 | googlenet | mobilenet"
        self.prune_percent  = prune_percent
        self.prune_iterations = prune_iterations
        
args = argument(end_iter =50,arch_type ="vgg11",dataset = 'cifar10',prune_percent  = 20,lr= 0.1,prune_iterations = 1)
