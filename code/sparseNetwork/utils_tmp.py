...
def print_nonzeros(model, list_all):
    nonzero = total = 0
    list_ = []
    for name, m in model.named_modules():
        if isinstance(m, PrunedConv):
            tensor = m.conv.weight.cpu().detach().numpy()
            nz_count = np.count_nonzero(tensor)
            total_params = np.prod(tensor.shape)
            nonzero += nz_count
            total += total_params
            print(f'{name:20} | nonzeros = {nz_count:7} / {total_params:7} ({100 * nz_count / total_params:6.2f}%) | total_pruned = {total_params - nz_count :7} | shape = {tensor.shape}')
            list_ += [100 * nz_count / total_params]
            
        if isinstance(m, PruneLinear):
            tensor = m.linear.weight.cpu().detach().numpy()
            nz_count = np.count_nonzero(tensor)
            total_params = np.prod(tensor.shape)
            nonzero += nz_count
            total += total_params
    ...
    return (round((nonzero/total)*100,1)), list_all

def original_initialization(mask_temp, initial_state_dict):
    global model,list_all
