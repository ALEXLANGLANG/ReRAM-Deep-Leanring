...
#    initial_state_dict = copy.deepcopy(model.state_dict())
    ...
    for args.arch_type in ["alexnet","vgg11", "vgg16","vgg19", "resnet18"]:
        print(f"************************{args.arch_type}{args.dataset} ************************")
        list_acc =[]
        list_sp =[]
        range_ = 30
        if args.arch_type ==  "vgg11":
            range_ = 25
    
        for iter_ in range(range_):
            ...
            sp = utils.get_Sparsity(model_tmp)
            list_sp.append(sp)
    #        res = utils.print_nonzeros(model_tmp,[])
        print(list_acc)
        print(list_sp)
        print(f"************************{args.arch_type}{args.dataset}************************")
#        get_weights("weights_" + args.arch_type + args.dataset + ".dat", model_tmp)
...
args = argument(arch_type ="vgg11",dataset="cifar10",prune_iterations = 25,prune_percent  = 20)
run_main(args)
