...
def train_(train_set,test_set,layer_name, q, batch_size_train = 128, momentum = 0, mask_bit_position = [1]*16, epochs = 10):
    torch.manual_seed(1)
    ...
    optimizer = optim.SGD(network.parameters(), lr=0.01, momentum = momentum, weight_decay=0.0005)
    ...
    for layer_name in list_layers:
        for q in list_q:
            for j in list_mask:
                ...
                acc = train_(train_set,test_set,layer_name=layer_name,momentum = momentum, mask_bit_position = mask_bit_position, q=q,epochs = 10)
                list_acc += [acc]
            print(list_acc)
...
print(df)
## save to xlsx file
filepath = './results/lent_weights_bit0_momentum.csv'
