...
def train_(train_set, test_set, layer_name, q, batch_size_train=128, momentum=0, mask_bit_position=[1] * 16, lr=0.01,
           epochs=10):
    torch.manual_seed(1)
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size_train, shuffle=False, pin_memory=True,
                                               num_workers=2)
    ...
    for layer_name in list_layers:
        for q in list_q:
            for j in list_mask:
                ...
                acc = train_(train_set, test_set, layer_name=layer_name, batch_size_train=batch_size_train,
                             mask_bit_position=mask_bit_position, q=q, epochs=10)
                list_acc += [acc]
        print(list_acc)
