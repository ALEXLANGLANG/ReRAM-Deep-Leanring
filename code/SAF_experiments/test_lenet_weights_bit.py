...
def train_(train_set,test_set,layer_name, q, mask_bit_position = [1]*16, epochs = 10):
    ...
    optimizer = optim.SGD(network.parameters(), lr=0.02, weight_decay=0.0005)
...
for layer_name in list_layers:
    for q in list_q:
        for j in list_mask:
            ...
            acc = train_(train_set,test_set,layer_name=layer_name,mask_bit_position = mask_bit_position, q=q,epochs = 30)
            list_acc += [acc]
...
filepath = './results/lenet_weights_bit.csv'
